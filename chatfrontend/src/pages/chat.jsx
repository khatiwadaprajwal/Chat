...
const ChatPage = () => {
    ...
    const handleMessage = (msg) => {
      ...
      if (
        ...
        setMessages((prev) => {
          if (
            prev.some(
              (m) => m.createdAt === msg.createdAt && m.text === msg.text
            )
          )
            return prev;
          return [...prev, msg];
          ...
          setFriends((prevFriends) =>
            prevFriends.map((f) => {
              if (
                ...
                String(f.id) === String(selectedFriendRef.current.id)
              ) {
                return {
                  ...f,
                  lastMessage: "",
    ...
    setFriends((prevFriends) => {
      const updatedList = prevFriends.map((friend) => {
        ...
        if (friendId === senderId || friendId === receiverId) {
          ...
          const isActive =
            selectedFriendRef.current &&
            String(selectedFriendRef.current.id) === friendId;
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const res = await fetch(`${BASE_URL}/messages/${selectedFriend.id}`, {
          headers: getAuthHeader(),
        });
  ...
  const callUser = async (isVideo) => {
    ...
    socketRef.current.on("callAccepted", async (signal) => {
      ...
      socketRef.current.emit("toggleMedia", {
  ...
  const leaveCall = () => {
    ...
    if (callAccepted && callStartTime.current) {
      ...
      if (
        selectedFriend &&
        (String(selectedFriend.id) === String(targetId) ||
          String(caller) === String(selectedFriend.id))
      ) {
  ...
  const handleDeleteConversation = async () => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/messages/conversation/${selectedFriend.id}`,
        {
          method: "DELETE",
          headers: getAuthHeader(),
        }
      ...
      if (res.ok) {
        setMessages([]);
        socketRef.current.emit("deleteAllConversation", {
          receiverId: selectedFriend.id,
        });
        setFriends((prev) =>
          prev.map((f) =>
