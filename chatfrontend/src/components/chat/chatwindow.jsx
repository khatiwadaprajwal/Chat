...
const ChatWindow = ({ 
  ...
  return (
    <div className="flex flex-col flex-1 bg-slate-50 relative h-full w-full">
      {/* HEADER */}
      <div className="bg-white/90 backdrop-blur-md px-3 py-2 md:px-4 md:py-3 border-b border-gray-100 flex justify-between items-center z-10 sticky top-0 shadow-sm w-full">
        
        {/* Left Side: Back + Pic + Name (Restricted width to prevent overflow) */}
        <div className="flex items-center gap-2 md:gap-3 overflow-hidden flex-1 min-w-0 mr-2">
          <button onClick={onBack} className="md:hidden p-1.5 -ml-1 text-gray-600 hover:bg-gray-100 rounded-full flex-shrink-0">
            <ChevronLeft size={22} />
          </button>
          
          <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold overflow-hidden border border-indigo-50 flex-shrink-0">
            {selectedFriend.profilePic ? (
                <img src={selectedFriend.profilePic} className="w-full h-full object-cover" alt="friend"/>
            ) : (
          ...
          <div className="flex flex-col min-w-0">
              {/* truncate class here ensures long names don't push buttons off screen */}
              <h3 className="font-bold text-gray-800 text-sm md:text-base leading-tight truncate">
                  {selectedFriend.name}
              </h3>
              <span className="text-[10px] md:text-xs text-green-500 font-medium truncate">Active Now</span>
          </div>
        </div>

        {/* Right Side: Icons (Fixed width, won't shrink) */}
        <div className="flex gap-1 md:gap-2 flex-shrink-0">
          <button onClick={() => onCallUser(false)} className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-full transition">
            <Phone size={18} className="md:w-5 md:h-5" />
          </button>
          <button onClick={() => onCallUser(true)} className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-full transition">
            <Video size={18} className="md:w-5 md:h-5" />
          </button>
          <button onClick={onDeleteConversation} className="p-2 text-red-500 bg-red-50 hover:bg-red-100 rounded-full transition ml-1" title="Delete Conversation">
      ...
      {/* MESSAGES LIST */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3 sm:p-6 w-full" style={{ backgroundImage: 'radial-gradient(#e2e8f0 1px, transparent 1px)', backgroundSize: '20px 20px', backgroundColor: '#f8fafc' }}>
        {messages.length === 0 && (
        ...
        {messages.map((msg, index) => {
            ...
            return (
              <div key={index} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300 group`}>
                <div className={`relative max-w-[80%] md:max-w-[60%] px-4 py-2 shadow-sm ${isMe ? 'bg-indigo-600 text-white rounded-2xl rounded-tr-sm' : 'bg-white text-gray-800 rounded-2xl rounded-tl-sm border border-gray-100'}`}>
      ...
      {/* INPUT AREA */}
      <div className="bg-white p-3 md:p-4 border-t border-gray-100 w-full">
        <form onSubmit={onSendMessage} className="flex items-end gap-2 max-w-4xl mx-auto bg-gray-50 p-1.5 rounded-[24px] border border-gray-200 focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-md transition-all">
          <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Message..." className="flex-1 py-2.5 px-4 bg-transparent outline-none text-gray-800 text-sm placeholder:text-gray-400 min-w-0" />
